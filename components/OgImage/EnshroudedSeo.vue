...
<template>
  <div
    class="w-full h-full flex flex-col justify-center bg-[#1d232a] text-[#a6adbb]"
  >
    <svg
    ...
    <div class="w-[600px] pl-[100px]">
      <p
        v-if="headline"
        class="uppercase text-xl mb-4 font-semibold"
        :class="`text-[${theme}]`"
      >
        {{ headline }}
      </p>
      <h1
        class="w-[600px] m-0 text-5xl font-semibold mb-4 text-white flex items-center"
      >
        <span>{{ title }}</span>
      </h1>
      ...
      <p class="font-bold">&rarr; enshrouded.vercel.app</p>
    </div>
    <img :src="icon" class="absolute top-[160px] right-[150px]" v-if="icon" />
  </div>
</template>
